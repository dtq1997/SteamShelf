...
### 渐进式披露方法论（Progressive Disclosure）

**核心原则：功能数量不变，可见性分层。用户第一眼看到的元素越少，掌控感越强。**

**可见性三层模型：**
| 层级 | 触发方式 | 适用场景 | 示例 |
|------|----------|----------|------|
| L1 始终可见 | 直接展示 | 核心操作（≤5个） | 搜索框、账号名 |
| L2 按需出现 | 右键/点击/快捷键 | 常用但非必需 | 筛选面板、笔记操作子菜单 |
| L3 条件触发 | 状态满足时自动出现 | 上下文相关 | Cloud上传（有未上传时才显示）、AI控件（选中游戏后出现） |

**UI 膨胀审计流程（新增功能或界面感觉拥挤时主动触发）：**
1. 盘点：数当前区域的可见元素数量
2. 分层：每个元素归入 L1/L2/L3
3. 重组：L2 用 popup menu / cascade submenu / 快捷键替代；L3 用条件显示
4. 验证：所有功能可达 + 操作路径未变长

**具体手法（tkinter）：**
- 平铺按钮 → `tk.Menu` + `tk_popup`（点击弹出菜单）
- 相关操作 → `menu.add_cascade`（子菜单分组）
- 始终显示的控件 → `pack_forget()` + 条件 `pack()`
- 高频操作 → 绑定键盘快捷键（Cmd/Ctrl+Key）

**主动触发时机：**
- 用户要求新增 UI 元素时，先评估是否会导致当前区域 L1 元素超过 5-7 个
- 如果超过，主动建议重组而非直接堆叠，并引用本方法论
- 用户反馈「界面太复杂/太多按钮」时，立即启动审计流程
